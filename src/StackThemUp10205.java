

import java.util.Scanner;

public class StackThemUp10205 {

	static String[] deckOfCards = { "2 of Clubs", "3 of Clubs", "4 of Clubs", "5 of Clubs", "6 of Clubs", "7 of Clubs", "8 of Clubs", "9 of Clubs", "10 of Clubs", "Jack of Clubs", "Queen of Clubs", "King of Clubs", "Ace of Clubs", "2 of Diamonds", "3 of Diamonds", "4 of Diamonds", "5 of Diamonds", "6 of Diamonds", "7 of Diamonds", "8 of Diamonds", "9 of Diamonds", "10 of Diamonds", "Jack of Diamonds", "Queen of Diamonds", "King of Diamonds", "Ace of Diamonds", "2 of Hearts", "3 of Hearts", "4 of Hearts", "5 of Hearts", "6 of Hearts", "7 of Hearts", "8 of Hearts", "9 of Hearts", "10 of Hearts", "Jack of Hearts", "Queen of Hearts", "King of Hearts", "Ace of Hearts", "2 of Spades", "3 of Spades", "4 of Spades", "5 of Spades", "6 of Spades", "7 of Spades", "8 of Spades", "9 of Spades", "10 of Spades", "Jack of Spades", "Queen of Spades", "King of Spades", "Ace of Spades" };
	static String[] oldDeckOfCards;
	static String[] newDeckOfCards = new String[52];
	static int[][] shuffles;

	public static void main(String[] args)    {

		Scanner input = new Scanner(System.in);
		int testCases, shufflesNo;

		String line;

		testCases = input.nextInt();
		
		for (int i = 0; i < testCases; i++) {
			initialize();
			shufflesNo = input.nextInt();

			shuffles = new int[shufflesNo][52];

			for (int j = 0; j < shufflesNo; j++) {

				for (int j2 = 0; j2 < 52; j2++) {
					shuffles[j][j2] = input.nextInt();
				}

			}


			 input.nextLine();
			while (input.hasNextLine()) {
				line = input.nextLine();
				if (line.equals("")) {
					break;
				}
				performShuffle(Integer.parseInt(line));

				
			}

			printAnswer();

			if (i != (testCases - 1)) {
				System.out.println("");
			}
		}
		input.close();
	}

	private static void initialize() {
		oldDeckOfCards = deckOfCards;
		newDeckOfCards = deckOfCards.clone();
	}

	private static void printAnswer() {
		for (int i2 = 0; i2 < newDeckOfCards.length; i2++) {
			System.out.println(newDeckOfCards[i2]);
//			if (i2 != (52 - 1)) {
//				System.out.print("\n");
//			}
		}

	}

	private static void performShuffle(int shuffleNumber) {

		for (int i3 = 0; i3 < 52; i3++) {
			
//			newDeckOfCards[shuffles[shuffleNumber-1][i3]-1]= oldDeckOfCards[i3];
			newDeckOfCards[i3]= oldDeckOfCards[shuffles[shuffleNumber - 1][i3] - 1] ;
		}
		
		oldDeckOfCards=newDeckOfCards.clone();
	}
}
